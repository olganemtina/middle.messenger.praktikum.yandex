...
function getData(target: MessagePanel, propertyKey: string, descriptor: PropertyDescriptor) {
...
export default class MessagePanel extends Block {
  ...
  protected initChildren(): void {
    this.children.card = new Card({
      ...
      description: this.currentChat?.status == State.Online ? 'онлайн' : 'офлайн',
      className: 'chat-item width-90-percent',
    });

    this.children.settings = new ImageButton({
      containerClassName: 'position-absolute top-35-percent right-15-px settings-button-container cursor-pointer',
      imgClassName: 'more-img cursor-pointer',
    });

    this.children.messages = new Array<MessageItem>();
    this.currentChat?.messages.map((message) => {
      (this.children.messages as Array<MessageItem>).push(new MessageItem({
    ...
    this.children.sendBtn = new ImageButton({
      ...
      events: {
        click: (evt: Event) => {
          let data = {
            messageText: ((this.children.messageInput as Block).props as any).value,
          };
