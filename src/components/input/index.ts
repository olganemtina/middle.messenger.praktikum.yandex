...
export default class Input extends Block {
  ...
  setValue() {
    (this.props as InputProps).value = (
      this.element.firstElementChild as any
    ).value;
  }

  setFocus() {
    (this.element.querySelector("input") as HTMLInputElement).classList.remove(
      "error"
    );
    let errorTextDiv = this.element.querySelector(
